-- The CXELLD-ZEE ↔ QUEENZEE WORKFLOW PROTOCOL (spec: cxell zees).
--
-- A cxell zee runs `claude --bare` inside a per-xell zee-agent container: no docker CLI, no host
-- filesystem, no skills. The queenzee API is its ONLY door out of the cxell. Every "skill" a
...
-- (1) Per-xell identity token. At cxell spawn the queenzee mints a random token, injects the
-- PLAINTEXT into the cxell env as ZEEHIVE_XELL_TOKEN, and stores only its SHA-256 HASH here. This is
-- IDENTITY, not a secret the cxell must protect (the cxell already cannot escape) — it lets the
-- queenzee know WHICH xell is calling and scope every verb to that xell. The plaintext is never
-- stored; a re-cxell xell simply mints a fresh one.
ALTER TABLE xell ADD COLUMN IF NOT EXISTS self_token_hash text;

-- One token identifies at most one xell. Partial: the column is NULL for every non-cxell xell.
CREATE UNIQUE INDEX IF NOT EXISTS xell_self_token_hash_uq
  ON xell (self_token_hash) WHERE self_token_hash IS NOT NULL;

-- (2) A cxell zee can only ASK to be bound to the PRODUCTION stack — it cannot bind itself. Binding
...
-- only on confirm does the queenzee attachProdStack AND re-seal the cxell firewall to allow the prod
-- db host:port. Until confirmed the cxell physically cannot reach prod.
