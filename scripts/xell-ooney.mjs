...
//                  [--db-ok "why the measured schema drift cannot break this change"]
//                  [--no-db]   (scope the ship to CODE ONLY — pending sql files are not applied)
//
...
const di = argv.indexOf('--db-ok');
const dbOk = di >= 0 ? argv[di + 1] : null;
...
const check = () => req('POST', '/api/ooney/check',
  { xell_id: xellId, targets, reason, db_ok: dbOk, skip_db: skipDb });
