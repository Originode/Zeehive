          ...
          <Section title={`history (${events.length})`}>
            {events.map((ev, i) => (
              <div key={ev.id || i} className="work-ev">
                <span className="work-ev-when">{fmtWhen(ev.created_at || ev.at)}</span>
...
// An event row reads as a sentence. `kind`/`field`/`from`/`to` are rendered when present and the
// raw kind is the fallback — a history that hides an event type it does not recognise is a history
// you cannot trust.
function describeEvent(ev, statuses) {
  const k = ev.kind || ev.type || 'changed';
  if (ev.field === 'status' || k === 'status') {
    return `moved it ${ev.from ? `from ${statusLabel(statuses, ev.from)} ` : ''}to ${statusLabel(statuses, ev.to)}`;
  }
  if (ev.field) return `changed ${ev.field}${ev.to != null ? ` to ${ev.to}` : ''}`;
  return ev.detail || ev.message || k;
}
...
async function countDescendants(itemId, projectId, data) {
  const given = data?.descendant_count ?? data?.item?.descendant_count;
