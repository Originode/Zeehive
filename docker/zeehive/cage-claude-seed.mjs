#!/usr/bin/env node
// Pre-answer Claude Code's first-run gauntlet INSIDE the cage so a human ATTENDING a caged zee
// over the dashboard terminal (ws → ssh → tmux → `claude`) never sees onboarding/theme/trust/
// bypass prompts — the cage is a fixed, trusted environment we control.
//
...
// it is safe to bake at image build AND re-run at spawn (cage-sshd.sh) as a belt-and-suspenders.
//
...
// permissions acknowledgement the attach uses (the cage is the permission system).
import { readFileSync, writeFileSync } from 'node:fs';
import { homedir } from 'node:os';
import { join } from 'node:path';

const path = process.argv[2] || join(homedir(), '.claude.json');
const PROJECT = process.env.CAGE_PROJECT || '/work/repo';
...
proj.hasTrustDialogAccepted = true;         // "Is this a project you trust?" for the cage cwd
