#!/bin/bash
# Open the cxell's SSH door so a human can ATTEND the zee — the dashboard terminal (ssh2→xterm)
# and Claude Code desktop both come in this way. Run as root (docker exec -u 0) after the cxell
# is up; idempotent. Args via env:
#   CXELL_PUBKEY  — the Zeehive public key authorized for user `zee`
#   CXELL_ENV     — lines to drop in /etc/environment so an SSH (PAM) login shell inherits the
...
ssh-keygen -A
cat > /etc/ssh/sshd_config.d/cxell.conf <<'EOF'
...
if [[ -n "${CXELL_PUBKEY:-}" ]]; then
  install -o zee -g zee -m 700 -d /home/zee/.ssh
  echo "$CXELL_PUBKEY" > /home/zee/.ssh/authorized_keys
...
# has already written) — the image bakes this too, but re-running here keeps a cxell seeded even if
# it was built from an older image. Written as root, then handed back to zee.
if [[ -x /usr/local/bin/cxell-claude-seed.mjs ]]; then
  node /usr/local/bin/cxell-claude-seed.mjs /home/zee/.claude.json || true
...
/usr/sbin/sshd
echo "cxell sshd up (key-only, user zee)"
