...
**A spinoff compose may itself be a GENERATED projection of the manifest**
([`compose-authorship-decision-record.md`](compose-authorship-decision-record.md)):
`lib/compose-gen.js` emits it from `zeehive.yml`, first line marked `# GENERATED by ZEEHIVE`.
A marked file is ZEEHIVE's to regenerate (`POST /projects/:id/compose/generate`); any compose
WITHOUT the marker is the project's own — onboarding absorbs it, ZEEHIVE never overwrites it.
Generated files are standalone-runnable without the harness (every parameter defaults) and
carry NO machine facts — placement stays meta-DB data, and the same file runs on any daemon
via `docker --context`. Zeehive's own `docker-compose.spinoff.yml` is such a projection.
