# Operator message
_sent 2026-08-08T18:14:10.703Z by human@console_

# Hierarchical Workflow Model — Architecture

**Status:** Design complete, unimplemented
**Companion:** `schema.sql` (PostgreSQL 15+ DDL)
...
- **Containment** — "prep ingredients" *is made of* wash + slice. A part-of relation. Forms a tree.
- **Precedence** — wash *comes before* slice. An ordering relation.

Both get drawn as arrows on a whiteboard. They are different edge types with different algebra, and they must be stored separately.

Three established traditions each solve part of this and none solves all of it:

| Tradition | Contributes | Cannot express |
|---|---|---|
| **Work Breakdown Structure** (project mgmt) | hierarchy, rollup, the 100% rule | branching, loops, runtime decisions |
| **Process trees / BPM** (van der Aalst) | operators, soundness by construction | non-series-parallel orders |
| **HTN planning** (SHOP2, SIPE) | alternative decompositions chosen by world state | nothing — but it is a *generator*, not a representation |

The impossibility that shapes this design is exact. Block-structured trees using sequence and parallel correspond precisely to **series-parallel partial orders**, and by the Valdes–Tarjan–Lawler theorem a partial order is series-parallel iff it contains no induced **N** pattern:

```
wash     → chop
marinate → chop
marinate → preheat
(wash and preheat unrelated)
```

No bracketing of these four tasks preserves exactly those constraints. Group `{wash, marinate}` then `{chop, preheat}` and you invent `wash ≺ preheat`. So process trees are a *strict subset* of DAGs, and the boundary is sharp and detectable.

**The design consequence:** DAG-over-tree is the substrate because it is the most general. Process-tree operators are layered on top as an *annotation* that buys back guarantees where they apply. HTN sits above as a generator that emits the other two.
...
1. Represent hierarchy and precedence without conflating them
2. Express anything a DAG can, while making block-structured regions cheap to validate
3. Type-check data flow at plan time, not run time
4. Treat every worker uniformly — person, model, script, service, sub-plan
5. Survive crashes, retries, and multi-day human waits with exact resumption
6. Support scheduling analysis (critical path, slack, capacity) on the same structure that executes

**Non-goals**

- Optimal resource-constrained scheduling — that is RCPSP, NP-hard; heuristics only
- Distributed transactions — compensation (saga) instead
- Being a general graph database — this is a task graph with strong opinions
...
```
┌────────────────────────────────────────────────────────────────────┐
│ PLANE 0  DEFINITION      plan · plan_version · template · method   │
│          version-controlled, immutable once published              │
└──────────────────────────────┬─────────────────────────────────────┘
                               │ expands into
┌──────────────────────────────▼─────────────────────────────────────┐
│ PLANE 1  PLAN            work_node · dependency                    │
│          mutable, editable, schedulable                            │
└──────────────────────────────┬─────────────────────────────────────┘
                               │ carries
┌──────────────────────────────▼─────────────────────────────────────┐
│ PLANE 2  DATA            port · data_edge · reducer                │
│          typed, checked at plan time                               │
└──────────────────────────────┬─────────────────────────────────────┘
                               │ executed as
┌──────────────────────────────▼─────────────────────────────────────┐
│ PLANE 3  RUN             run · execution · lease · allocation      │
│                          checkpoint · event                        │
│          append-only, durable, replayable                          │
└────────────────────────────────────────────────────────────────────┘

        ╔════════════════════════════════════════════════╗
        ║ CROSS-CUTTING   entity · entity_member         ║
        ║ anything that can be assigned work             ║
        ╚════════════════════════════════════════════════╝
```

**Referencing rule: a plane may reference the plane below it, never above.** A `work_node` points at its `template`; a `template` never points at a `work_node`. An `execution` points at a `work_node`; a `work_node` never points at an `execution`.

This is what lets you re-plan without corrupting run history, and replay a run against the exact plan version it used.

---

## 4. work_node

One table for containers and leaves (Composite pattern). This is what makes rollup, dependencies, scheduling, and templates operate uniformly instead of forking into two code paths.

`kind` is deliberately minimal — `container | action | signal`. There is no `wait` kind and no `subprocess` kind:

- **Waiting** is a long lease held by a pull entity (§6)
- **Subprocess** is an action bound to a plan-backed entity (§6)

Both were special cases that dissolved once entities were unified.

### 4.1 The operator set

`child_semantics` is the single field doing the BPM work. Ordering, completion rule, duration formula, and edge legality are all functions of it.
...
| `parallel` | all, concurrent, static width | all done | max children | no — contradictory |
| `choice` | one, selected by `guard` | selected done | max (worst case) | no — branches exclusive |
...
| `try` | body; catch on error | per `try_role` | body (+ catch path) | no |
| `freeform` | per union-graph edges | all done | CPM longest path | **yes** |

**`map`** is dynamic fan-out — width unknown until runtime. `map_spec.over` evaluates against scope; each item instantiates the child subtree with the item bound; `concurrency` caps parallelism; `reducer` merges instance outputs. This is the single most-missed feature in hand-rolled workflow models.

**`race`** is first-wins with cancellation. Covers timeout-vs-work, redundant providers, and "whichever reviewer responds first." Without it people fake it with `parallel` plus manual cancellation and get it wrong.

**`try`** is a scoped error boundary. Children are tagged `body | catch | finally | compensate`. Compensation is the saga pattern: on failure after partial completion, walk completed `body` children **in reverse** and run each one's `compensate` sibling. Transactional semantics without distributed transactions.

**`loop` and `map` are single vertices in their parent's graph.** Their bodies are contracted, so repetition never makes the union graph cyclic. Same trick structured programming used against `goto`.

### 4.2 The edge legality invariant

> **An explicit `dependency` is legal iff `LCA(from, to).child_semantics = 'freeform'`.**

Rationale per case: under `sequence` the edge is already implied or contradicts rank. Under `parallel` it contradicts parallelism. Under `choice` or `race` only one branch survives, so a cross-branch edge is incoherent.

What this buys:

- **Non-freeform subtrees are sound by construction.** No analysis. No deadlock. No orphaned tokens. Well-formedness *is* validity.
- **Cycle detection and topological sort run only inside `freeform` regions.** On a mostly-structured plan this is a large constant-factor win.
- **The N pattern remains expressible** — put those four tasks under a `freeform` parent. Full generality preserved; validation cost paid only where generality was used.

Keep `freeform` rare. It is the only construct without structural guarantees, and every invariant you cannot check statically becomes a runtime incident.

### 4.3 Policy inheritance

`retry_policy`, `timeout`, `on_error`, `priority`, and `calendar_id` resolve by walking up to the nearest ancestor that sets them (`wn_effective_policy()`).

Without this, the model is unusable at scale: you would configure four hundred leaves instead of one root and three overrides.

---

## 5. Data plane

Making data first-class risks introducing a third edge type. It does not, because of one rule:

> **A data edge implies a precedence edge. Always. Automatically.** (I14)

If `B.input.x` is bound to `A.output.y`, then `A ≺ B`.

Consequences:

- `dependency` rows now mean only **control-only** precedence — "after that, for reasons the data does not capture." Most real dependencies become data edges and the ordering derives itself.
- This kills the most common bug class in workflow tools: wiring the data but forgetting the ordering edge, or reordering tasks and silently breaking a flow.

**Reducers are mandatory** wherever a port has multiple incoming edges — which is every `parallel` and `map` fan-in. Without them, concurrent writes clobber each other nondeterministically. The schema rejects a multi-source port with no reducer (I12): a plan-time error, not a runtime surprise.

**Type checking at plan time.** A `data_edge` is valid iff `from_port.type` is assignable to `to_port.type`. A whole class of failure is caught before anything runs. This is the payoff for making data first-class.

**Scoping is lexical, innermost wins.** A node reads from its own input ports → enclosing `scope_vars` chain → run globals. Under `map`, the loop variable binds in the instance's scope, which is what makes concurrent instances isolated by construction.

---

## 6. The entity model

> **An entity is anything that can be assigned work and eventually return a result.**

A person, a model, a script, a service, a pool, an org unit, another plan. The differences between them are not differences in *kind* — they are differences in *parameter values*: latency, reliability, cost, how you cancel them. Columns, not classes.

`kind_hint` exists so dashboards can show an icon. **The moment engine logic branches on it, the abstraction has failed.**

### 6.1 The uniform contract

```
assign(work_item) → lease
poll(lease)       → running | done(output) | failed(error)
cancel(lease)     → per entity.cancellable
```

A script gets `assign` as a subprocess spawn. A service gets an HTTP call. A model gets an inference request. A person gets a row in their task inbox. **The engine cannot tell these apart** — it holds a lease and waits.

### 6.2 Lease-based dispatch

This is what makes the collapse real rather than cosmetic.

| | Push entity | Pull entity |
|---|---|---|
| Dispatch | engine calls out | work lands in a queue, entity claims it |
| Lease length | seconds | days |
| Renewal | heartbeat | opening the task |
| Example | HTTP service, model call | person, batch worker |

**Lease expiry is the universal timeout.** A crashed worker and a person who went on holiday are the same event: the lease lapsed, requeue it. One timeout mechanism, one recovery path — and no `wait` node type, because every action is asynchronous and some just happen to resolve fast.

### 6.3 Capability matching

Nodes describe what they need, not who does it:

```
req_capabilities  ['translate', 'legal-domain']
req_constraints   'entity.reliability > 0.95'
req_selection     cheapest | fastest | most_reliable
                  | round_robin | least_loaded | explicit
```

That resolves to a human translator, a fine-tuned model, or a vendor API — chosen at dispatch time by policy. **Swapping a human step for an automated one becomes a capability registration, not a plan edit.** This is the practical payoff of the entity collapse and the main reason it is worth doing.

### 6.4 Entity absorbs Resource

A resource was already an entity with capacity and no output contract. `concurrency` + `calendar_id` + `rate_limit` covers what `Resource.capacity` did. One distinction survives, as a boolean:

- `consumable = false` — capacity returns on release (people, workers, licences). **Leased.**
- `consumable = true` — capacity spent permanently (budget, materials). **Debited.**

### 6.5 A plan is an entity

`entity.plan_id` set means assigning work to this entity **launches a workflow**. Your plan calls an entity; that entity is another plan; that plan assigns to its own entities. Recursion all the way down.

This is why `kind = subprocess` disappeared. It is just an action bound to a plan-backed entity.

**It also creates a second graph.** The entity call graph between plans needs its own cycle detection (`detect_plan_call_cycles()`, invariant E7), separate from the union graph inside each plan. Easy to miss.

### 6.6 Where the abstraction leaks

Three honest leaks. Keep them in the data, not in branching logic:

| Leak | Handling |
|---|---|
| **Cancellation** | `cancellable = never` means orphaned work exists. A `race` loser that cannot be stopped still consumes capacity and may still produce side effects. Track it (`orphaned_work` view); do not pretend it stopped. |
| **Retry semantics** | Retrying a nondeterministic entity may give a different answer; retrying a person is a social act. `deterministic` covers the first. For the second: `max_attempts: 1`, `on_error: escalate` — which is just a different capability requirement. |
| **Input shaping** | A model wants a prompt, a person wants a form, a service wants JSON. This lives in the entity adapter. If the plan knows what a prompt is, the collapse leaked. |
| **Cost units** | Tokens, minutes, and dollars are not commensurable. `selection: cheapest` needs a pluggable normalisation policy, not a pretend single number. |

---

## 7. The union graph

Scheduling and cycle detection **never** read the `dependency` table alone. Edges come from three sources:

1. Sibling order under `sequence`
2. Explicit `dependency` rows, expanded per link type (FS/SS/FF/SF)
3. Every `data_edge` (I14)

### 7.1 Leaf expansion is not optional

Contracting each subtree to one vertex reports **false cycles**. Counterexample:
...
contracted:  A → B  and  B → A          ✗ reported as a cycle
expanded:    A1 → B1 → B2 → A2          ✓ acyclic and schedulable
```

Users hit this within a week of shipping. Use contraction only as a cheap early-out that must never reject.

Link-type expansion:

| Type | From | To |
|---|---|---|
| FS | `last_leaves(from)` | `first_leaves(to)` |
| SS | `first_leaves(from)` | `first_leaves(to)` |
| FF | `last_leaves(from)` | `last_leaves(to)` |
| SF | `first_leaves(from)` | `last_leaves(to)` |

### 7.2 Scheduling — two phases, kept separate

**Phase 1 — unconstrained CPM.** Forward pass for earliest start/finish, backward pass for latest start/finish, slack = latest − earliest, critical path = zero-slack chain.

**Phase 2 — resource levelling.** Only pushes tasks later, never earlier.

Keeping them separate means the critical path stays interpretable, and you can show "here is the theoretical minimum; here is what your capacity does to it."

**Honest warning:** once nodes compete for limited entities, this is RCPSP and it is NP-hard. Use priority-rule heuristics (serial SGS with latest-finish-time priority) and label the output **"feasible schedule," not "optimal."**

Critical path runs through *leaves* and ignores the hierarchy. Containers report the span of their subtree, not a path role.

### 7.3 Estimates as distributions

If `estimate` is polymorphic from day one (`interval` or PERT/triangular `jsonb`), you get Monte Carlo over the network and can report P50/P80 instead of a single fictional date. Cheap now; painful to retrofit onto a scalar column.

---

## 8. Run plane

### 8.1 Execution is per-attempt

One row per work_node **per attempt**. Retries, map instances, and loop iterations each get their own row. Reusing one row per node destroys history the moment anything retries.

### 8.2 Idempotency

Any node with external effects computes `effect_key` from inputs plus identity. Before executing, check whether that key already succeeded in this run; if so, replay the recorded output.

**This is not optional in any system that touches the outside world.** At-least-once delivery without it means duplicate charges and duplicate emails.

### 8.3 Replay

`checkpoint` + `event` gives resume-after-crash, time-travel debugging, and fork-from-step-N. Snapshot on a cadence; the event log fills the gaps. The event table is append-only, enforced by trigger.

### 8.4 State machine

```
pending ──ready?──► ready ──dispatch──► running ──┬──────────► done
   │                  │                    │      │
   │                  │                    │      ├──► failed ──retry──► ready
   │                  │                    │      │
   ...
   ├──► skipped       │                                      
   └──► blocked       └──► blocked  (no entity satisfies requirements)

              done ──rollback──► compensated
```

`ready` means: all union-graph predecessors satisfied under this node's `trigger_rule`, guard true, an entity resolvable, calendar open.

`compensated` is a **distinct terminal state**, not a flavour of done or failed. A partially rolled-back subtree must not read as either.

### 8.5 Skip propagation

When `choice` selects one branch, unselected branches become `skipped`. Anything downstream needs a rule or you get permanently stuck runs:

| `trigger_rule` | Behaviour |
|---|---|
| `all_success` | blocks on skipped or failed predecessors *(default)* |
| `none_failed` | runs if predecessors are done or skipped |
| `all_done` | runs regardless of outcome |
| `one_success` | runs as soon as any predecessor succeeds |

Pick the default deliberately. Airflow shipped `all_success` and has spent years explaining the resulting stuck DAGs.

---

## 9. Templates (the HTN role)

Strictly generative: emits Plane 1–2 structure, then has no runtime role.

The load-bearing feature is **multiple methods per template**. `buy_ingredients` decomposes to `[store → find → checkout]` or `[order_online → wait]` depending on `guard`. That is the one HTN capability with no process-tree or DAG equivalent.

```
expand(node):
  T ← template[node.template_id]
  M ← highest-priority method of T whose guard holds under node.bindings
  if none → mark primitive, or raise
  instantiate M.body as children (stamp template_id, method_id, bindings)
  node.expansion ← 'expanded'
  re-validate I5 and I10 on the affected region
```

Three restrictions that keep you out of EXPSPACE territory:

1. **Lazy expansion** — expand when a node becomes `ready`, or on user request. Never eagerly to the bottom.
2. **No backtracking** — first matching guard wins. If a guard later becomes false, mark the subtree `stale` and offer re-expansion as an explicit user action rather than silently replanning.
3. **Bounded recursion depth** — self-referencing templates are legal and useful, but must be capped and detected.

**`choice` and `method` are not redundant.** A method resolves at *expansion* time and produces structure. A `choice` resolves at *execution* time and lives in the structure. You need both.

---

## 10. Versioning

Plans change while runs are in flight. Pick per-run, not globally:

| Policy | Behaviour |
|---|---|
| `pinned` | run finishes against the version it started with *(safest default)* |
| `migrate` | apply the diff, but only to nodes still `pending` |
| `restart` | abandon and re-plan from current state |

`stable_key` gives node identity across versions. Seed it from template + path, **not row order**, or every reorder looks like a delete-and-insert.

---

## 11. Failure semantics matrix

Every operator × failure-mode pair. **Fill this table before implementing** — the cells you cannot fill are the ones to forbid at the schema level.

| Operator | One child fails | One child skipped | Cancel requested | Compensating |
|---|---|---|---|---|
| `sequence` | halt; remaining → `skipped`; propagate per `on_error` | continue per successor `trigger_rule` | cancel current, skip rest | reverse order over completed |
| `parallel` | others continue to completion, then parent fails | parent completes; downstream per `trigger_rule` | cancel all; honour `cancellable` | all completed children, any order |
| `choice` | parent fails (only one branch ran) | n/a — non-selected are skipped by design | cancel selected branch | selected branch only |
| `race` | that child loses; others continue | n/a | cancel all | **winner only**; losers were cancelled, never completed |
| `map` | per `map_spec.on_item_error`: `fail_fast` \| `collect` \| `skip` | n/a | cancel all instances | **each completed instance individually** |
| `loop` | exit loop, propagate | n/a | cancel current iteration, exit | reverse over completed iterations |
| `try` | route to `catch`; `finally` always runs | per role | cancel body, run `finally` | this is the compensation scope |
| `freeform` | successors per `trigger_rule` | successors per `trigger_rule` | cancel all unfinished | reverse topological order |

The two genuinely hard cells, called out because they are where hand-rolled engines break:

- **`map` + compensate** — each completed instance compensates independently, with its own item binding restored from the execution's snapshotted `inputs`. This is why `execution.inputs` must be snapshotted rather than recomputed.
- **`race` + compensate** — losers are `cancelled`, never `done`, so they do not compensate. But if the loser's entity was `cancellable = never`, it may have completed anyway and produced side effects. That work is in `orphaned_work` and needs a human decision. There is no correct automatic answer.

---

## 12. Build order

Each stage is independently useful. Each later stage is a real cost.

| # | Stage | Adds | Why here |
|---|---|---|---|
| 1 | **Core** | work_node tree, `sequence`/`parallel`/`freeform`, dependency, leaf-expanded cycle check, duration rollup | irreducible |
| 2 | **Data** | ports, data_edges, reducers, type check | unlocks I14 — most dependencies stop being manual |
| 3 | **Durability** | run, execution, event, idempotency keys | nothing is trustworthy without this |
| 4 | **Branching** | `choice`, `loop`, trigger rules, skip propagation | first real control flow |
| 5 | **Entities** | entity, lease, capability matching | replaces any hardcoded executor notion |
| 6 | **Scheduling** | CPM, slack, critical path, calendars | planning value |
| 7 | **Hardening** | retry, timeout, `try`/`catch`, policy inheritance | production |
| 8 | **Signals** | `signal` nodes, long-lease pull entities | long-running processes |
| 9 | **Dynamic** | `map`, `race`, concurrency caps | dynamic topology |
| 10 | **Replay** | checkpoints, time-travel, fork | debuggability at scale |
| 11 | **Capacity** | consumable entities, allocation, levelling | only when contention is real |
| 12 | **Saga** | compensation | only when partial failure is expensive |
| 13 | **Planning** | template/method, lazy expansion | only with genuine alternative decompositions |
| 14 | **Versioning** | plan versions, in-flight migration | only when plans change mid-run |

**Stages 1–5 cover roughly 90% of real workflows.** Stages 11–14 exist because someone's specific problem demanded them. Build them when that someone shows up.
...
| Risk | Mitigation |
|---|---|
| **Operator combinatorics.** `map` × `try` × `race` × retry is four features and a dozen unspecified edge cases. | Complete §11 before implementing. Forbid unfillable cells at the schema level. |
| **`freeform` becomes the default.** Users reach for it because it never complains, and you lose all guarantees. | Make it require an explicit choice in the UI. Report the freeform ratio as a plan-health metric. |
| **Contracted cycle detection.** Ships, seems fine, produces false cycles under nesting. | Leaf expansion in `union_edge` from day one. Test the A1→B1, B2→A2 case explicitly. |
| **`kind_hint` leaks into logic.** One `if entity.kind_hint = 'human'` and the collapse unravels. | Lint for it. It is display-only by contract. |
| **RCPSP presented as optimal.** Users trust a schedule that is one heuristic among many. | Label output "feasible." Show the unconstrained CPM alongside it. |
| **Recursive plan-entities.** Plan A calls B calls A; unbounded expansion at runtime. | `detect_plan_call_cycles()` on publish, plus a runtime depth cap. |
...
| Term | Meaning |
|---|---|
| **atom** | a union-graph vertex: any leaf, plus `loop`/`map` containers (bodies contracted) |
| **union graph** | the leaf-expanded precedence DAG assembled from sequence order + dependencies + data edges |
| **LCA** | lowest common ancestor; determines whether an explicit dependency is legal |
| **lease** | a time-bounded claim on an execution by an entity; expiry is the universal timeout |
| **effect key** | idempotency hash of inputs plus identity, deduplicating external side effects |
| **N pattern** | the minimal partial order that is not series-parallel; requires `freeform` |
| **saga** | compensation-based rollback: reverse-order undo of completed work |
| **superstep** | one scheduling round; all nodes that became ready together |
...
CREATE TABLE execution (
    ...
    UNIQUE (run_id, work_node_id, attempt,
