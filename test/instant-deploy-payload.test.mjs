...
try {
  ...
  // Pasted files ride the dispatch body as base64 data URLs, so an oversized attachment fails the
  ...
  // The wire field is `images` (legacy name) but it carries any file attachment.
  ...
  ok(/total > MAX_BYTES/.test(src) && /Attachments exceed 20 MB/.test(src)
     && /images: attachments\.map\(\(\{ name, data \}\) => \(\{ name, data \}\)\)/.test(src),
     'addFile refuses an oversized attachment with a named error, and attachments ride the payload');
} catch (e) {
