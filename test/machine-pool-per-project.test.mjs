// MACHINE-POOL-PER-PROJECT test — the contract of migration 025 ("Pool size is a project's
// appetite, not a machine's habit"): the warm-pool size is a (machine, project) fact, while the
// hard cap on live dev xells is MACHINE-WIDE (shared across every project). Before 025 one
// pool_size on the machine row applied to every project — Zeehive kept as many warm xells on a
// host as OmniBiz. This test would FAIL the moment pool size slips back to machine-wide.
//
...
const { setMachinePool, machinePoolSize, listMachines, liveXellCount } =
