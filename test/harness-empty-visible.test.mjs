...
const ghostKey = `zt-ghost-${tag}`;      // file-backed, folder nowhere → files_missing
const hollowKey = `zt-hollow-${tag}`;    // DB-owned, nothing in the bundle → bundle_empty
const fullKey = `zt-full-${tag}`;        // a real persona, for the contrast
const KEYS = [ghostKey, hollowKey, fullKey];
const PID = `00000000-0000-4000-8000-0000000eb${tag.slice(0, 3)}`.slice(0, 36);
let projId = null, xellId = null;

try {
  // ── fixtures: one harness of each kind, and a live xell WEARING the empty one ───────────────
  const ghost = await one(
    `INSERT INTO harness (key,label,dir,enabled,is_law_core) VALUES ($1,$2,$3,true,false) RETURNING id`,
  ...
  xellId = (await one(
    ...
    [projId, xource.id, `hev-${tag}`, ghost.id])).id;

  // ── 1. the boot readiness line ──────────────────────────────────────────────────────────────
  console.log('\n── refreshHarnesses() ends with ONE line a human can read ──');
  const before = recentLogs(500).length;
  const sum = await H.refreshHarnesses();     // the real boot path
  ...
  ok(!!summary, `the refresh emits a summary line (${summary || 'NONE — the whole point'})`);
  ok(/\d+ loaded, \d+ EMPTY/.test(summary || ''), 'it counts both halves: N loaded, M EMPTY');
  ok((summary || '').includes(ghostKey), 'and NAMES the empty harness, so nobody has to go hunting');
  ok(/no files/.test(summary || ''), 'saying which are empty because their FILES are unreadable');
  ...
  ok(!(summary || '').includes(fullKey), 'a healthy file-backed harness is not named in the empty list');
  ...
  ok(byKey[ghostKey]?.files_missing === true && byKey[ghostKey]?.bundle_empty === true, 'ghost: files_missing + bundle_empty');
  ok(byKey[hollowKey]?.bundle_empty === true && byKey[hollowKey]?.files_missing === false,
     'hollow (DB-owned, nothing in it): bundle_empty, but its files are not "missing" — it has none to miss');
  ...
  const tlH = (tl?.harnesses || []).find((h) => h.key === ghostKey);
  ok(!!tlH && tlH.files_missing === true,
     'getTimeline() emits files_missing/bundle_empty on the harness a xell wears (the badge reads this)');
  ...
  ok(emptyWarning({ files_missing: true, bundle_empty: true }).chip === '⚠ no files',
     'files_missing wins over bundle_empty — it names the CAUSE');
  ...
  const ghostRow = row({ key: ghostKey, label: 'Ghost', skill_count: 0, file_backed: true, files_missing: true, bundle_empty: true });
  const hollowRow = row({ key: hollowKey, label: 'Hollow', skill_count: 0, file_backed: false, bundle_empty: true });
  const fullRow = row({ key: fullKey, label: 'Full', skill_count: 1, file_backed: false });
  ok(ghostRow.includes('⚠ no files'), 'the list row for an unreadable harness SAYS "no files"');
  ok(hollowRow.includes('⚠ empty'), 'and an empty one SAYS "empty" — a word, not a shade');
  ...
  ok(/title="[^"]*not readable[^"]*"/.test(ghostRow), 'the row explains itself on hover');
  ok(ghostRow.includes('hm-hollow') && ghostRow.includes(`data-testid="harness-empty-${ghostKey}"`),
  ...
  const ghostBanner = banner({ key: ghostKey, file_backed: true, files_missing: true, bundle_empty: true });
  ok(ghostBanner.includes('harness-empty-banner') && /carries no files/.test(ghostBanner),
     'the EDITOR banner states it outright when you open that harness');
  ok(ghostBanner.includes(`harnesses/${ghostKey}/`) && /repo_root|checkout/.test(ghostBanner),
     'and tells an operator WHERE the files were supposed to come from');
  ok(banner({ key: hollowKey, bundle_empty: true }).includes('carries nothing'),
     'an empty DB-owned persona gets the same treatment in different words');
  ...
  ok(hive.harnessWarning({ files_missing: true }) === '⚠ no files'
     && hive.harnessWarning({ bundle_empty: true }) === '⚠ empty'
     && hive.harnessWarning({}) === null,
     'the canvas exports the same verdict for the same two fields');
  ...
  ok(drawnFor({ files_missing: true }).includes('⚠ no files'),
     'the honeycomb BADGE writes "⚠ no files" where the ×N count goes');
  ok(drawnFor({ bundle_empty: true }).includes('⚠ empty'), 'and "⚠ empty" for an empty bundle');
