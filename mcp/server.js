#!/usr/bin/env node
// XEEHIVE MCP server — gives an agent structured tools (an alternative to the /xell skills)
...
const server = new McpServer({ name: 'xeehive', version: '0.1.0' });

server.tool('xeehive_get_context',
...
server.tool('xeehive_report_done',
...
server.tool('xeehive_prod_lock_acquire',
...
server.tool('xeehive_prod_lock_release',
...
server.tool('xeehive_prod_lock_status', 'Who holds the production deploy lock right now?', {},
