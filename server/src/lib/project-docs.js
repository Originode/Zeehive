...
// ── the inverse of renderDoc: recover the BODY from a generated file ──────────────────────────────
// A generated provider file is [frontmatter?] + banner + body + [xell stack]. Getting the body back
// out is what lets a check compare the two copies of a project's one document without re-deriving the
// wrapper by counting lines — `sed 1,8d` is right until the banner gains a line, and then it is
// silently wrong. Keyed on the markers the generator itself writes (ticket #38).
//
// Returns null when the text is not a generated file at all, so a caller can tell "not generated"
// from "generated and empty" instead of comparing against an accidental empty string.
// The first line of the appendix xell-stack.js writes. Keyed on the generator's own text, so a change
// there breaks this loudly rather than silently shifting what counts as "the body".
const STACK_HEADING = '## Your xell — the stack you actually own';

export function extractGeneratedBody(text) {
  const s = String(text || '');
  const bannerEnd = s.indexOf('-->');
  if (!s.includes('GENERATED by ZEEHIVE') || bannerEnd < 0) return null;
  let body = s.slice(bannerEnd + 3);
  // The per-xell stack appendix opens with a rule and its own H2 (lib/xell-stack.js), then its own
  // stamp. Cut at the HEADING, not at the stamp: cutting at the stamp leaves the rule and the heading
  // behind, which is the kind of almost-right that makes a comparison fail for a reason nobody can see.
  const stackAt = body.indexOf(STACK_HEADING);
  if (stackAt >= 0) body = body.slice(0, stackAt).replace(/\n+---\s*$/, '');
  return body.trim();
}
