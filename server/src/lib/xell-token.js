// PER-XELL IDENTITY TOKEN — the credential a CAGED zee presents to the queenzee's /api/xell/self/*
// verbs so the server knows WHICH xell is calling and scopes every action to it.
//
// This is IDENTITY, not security: the cage is the wall (default-DROP egress, no docker, no host fs),
...
// the plaintext exists only inside the cage's environment as ZEEHIVE_XELL_TOKEN.
...
// the caller injects it into the cage env). Overwrites any prior token: a re-caged xell re-identifies.
