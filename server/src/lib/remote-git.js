...
// RECONCILE local <branch> TO the remote's <branch> — the "make the ordinary PR work again" verb.
//
// A fast-forward Pull refuses a DIVERGED local (both sides moved — which is exactly the shape of
// the squashed-snapshot case: the remote was squashed clean while the local branch kept its full
// history, so every PR re-pushes a dirty range and GitHub refuses it by rule). The console's other
// remedy — Xource Clean — resets to the LOCAL tip, which is the dirty tip, so it cannot un-wedge
// the PR either. This verb is the missing third: re-point the local branch at the recorded remote's
// clean tip, so an ordinary (non-squashed) PR works again.
//
// It is deliberately NARROW:
//   • only ever resets the checked-out main branch to origin/<main> — it does not touch any other
//     ref, worktree, or xell, and it never force-pushes;
//   • refuses when the tree is dirty (same stance as pullRemote: never discard uncommitted work);
//   • refuses when there is nothing to reconcile (local == remote: up-to-date) or when local is
//     strictly BEHIND the remote (that is a fast-forward Pull, not a reconcile);
//   • it is a HUMAN action (the console's ⟲ Reconcile button, behind a confirm dialog) and the
//     project verb that calls it is PROVISION_MODE-gated like every other outbound verb.
//
// Local commits NOT on the remote are DISCARDED by this verb — that is the point (they are the
// dirty range), and it is exactly why a human confirms first.
export async function reconcileRemote({ repoRoot, branch = 'main', remoteUrl, token } = {}) {
  if (!repoRoot || !remoteUrl) return { reconciled: false, state: 'error', reason: 'repoRoot and remoteUrl are required' };
  const g = (args, opts = {}) => gitNet(['-C', repoRoot, ...args], opts);

  // the checkout must be on the branch we're resetting, or we'd re-point the wrong ref
  const cur = await g(['rev-parse', '--abbrev-ref', 'HEAD'], { timeout: 15000 });
  const current = cur.status === 0 ? cur.out.trim() : null;
  if (current !== branch) {
    return { reconciled: false, state: 'refused', reason: `checkout is on '${current || 'unknown'}', not '${branch}' — reconcile only re-points the checked-out main` };
  }

  // dirty tree → refuse (same stance as pullRemote and the landing gate: never discard uncommitted
  // work). TRACKED changes only — untracked files are not at risk from a reset --hard and git
  // itself aborts if it would need to write over one; .claude/worktrees are untracked by design.
  const st = await g(['status', '--porcelain', '--untracked-files=no'], { timeout: 30000 });
  const dirty = st.status === 0 ? st.out.split('\n').filter(Boolean).length : -1;
  if (dirty !== 0) {
    return { reconciled: false, state: 'refused', reason: dirty > 0 ? `${dirty} uncommitted change(s) in the checkout — commit or stash first` : 'could not read working-tree status' };
  }
  ...
  const remoteTip = (await g(['rev-parse', `origin/${branch}`], { timeout: 15000 })).out.trim();
  if (!remoteTip) return { reconciled: false, state: 'error', reason: `origin/${branch} did not resolve after fetch` };

  // nothing to reconcile: local already at the remote tip → up-to-date; local is a pure ancestor of
  // the remote → a fast-forward Pull would do it, a reset is the wrong (destructive) tool.
  if (remoteTip === before) return { reconciled: false, state: 'up-to-date', from: before, to: remoteTip, commits: 0 };
  const isAnc = await g(['merge-base', '--is-ancestor', before, `origin/${branch}`], { timeout: 15000 });
  if (isAnc.status === 0) {
    return { reconciled: false, state: 'refused',
      reason: `local ${branch} is behind origin/${branch} — use ↓ Pull (a fast-forward), not a reconcile` };
  }

  const reset = await g(['reset', '--hard', `origin/${branch}`], { timeout: 60000 });
  if (reset.status !== 0) {
    return { reconciled: false, state: 'error', reason: `reset --hard failed: ${(reset.err || '').trim().slice(-300)}` };
  }

  const after = (await g(['rev-parse', 'HEAD'], { timeout: 15000 })).out.trim();
  const dropped = await g(['rev-list', '--count', `origin/${branch}..${before}`], { timeout: 30000 });
  return {
    reconciled: true, state: 'reconciled', from: before, to: after,
    dropped: dropped.status === 0 ? (+dropped.out.trim() || 0) : null,
  };
}
