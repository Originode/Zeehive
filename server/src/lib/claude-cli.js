...
function run(args, timeout = 20000) {
  const r = spawnSync(CLAUDE, args, { encoding: 'utf8', timeout, windowsHide: true, shell: WIN });
...
export function listActiveAgents({ all = false } = {}) {
  try {
    const r = run(['agents', '--json', ...(all ? ['--all'] : [])]);
...
export function activeSessionIds(opts) {
  return new Set(listActiveAgents(opts).agents.map((a) => a.sessionId));
}
...
export function remoteList() {
  const r = run(['remote', 'list', '--json'], 15000);
  const text = `${r.stdout}\n${r.stderr}`;
...
export function remoteStatus(ref) {
  const r = run(['remote', 'status', ref], 12000);
  const text = `${r.stdout}\n${r.stderr}`;
