...
export const LANGCHAIN_TOOLS = {
  ...
  working: {
    ...
    description: 'Writes a "working" ping for THIS xell (a session_event + a zee status update) that '
      ...
      + 'semantics as every cxell zee. The LOOP policy (runLangchainAgentTurn) refuses to RUN this '
      + 'tool while a tend is open, so a model ping cannot silently clear a human\'s question — but '
      + 'the tool itself is the plain shared handler. Use it only when you are genuinely working, '
      + 'never as conversational filler.',
    schema: { type: 'object', properties: { note: { type: 'string' } }, required: [] },
    // PLAIN SHARED HANDLER — the tool IS the door to selfWorking, exactly as `zee working` is for a
    // cxell zee. Whether a langchain zee may reach for `working` while a tend is open is LOOP POLICY
    // (the same place loop-ends-turn-on-tend lives), not verb semantics — see runLangchainAgentTurn.
