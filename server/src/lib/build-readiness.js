...
//
// Every docker call is bounded and spawned exactly the way verifyRequires() does it — one thin
// adapter (dockerAdapter) so the whole probe is testable without a docker daemon.
...
// the way verifyRequires() already does; a test injects a stub here so the probe logic runs
// without docker. Resolves { status, stdout, stderr } when the CLI answered, or
// { unknown, reason } when it could not run at all (no docker binary, spawn error, timeout).
export function dockerAdapter(ctx, args, { timeout = BUILD_READINESS_TIMEOUT_MS } = {}) {
  let r;
  try {
    r = spawnSync('docker', ['--context', ctx, ...args],
      { encoding: 'utf8', timeout, windowsHide: true });
  } catch (e) {
    return { unknown: true, reason: e.message };
  }
  if (r.error) return { unknown: true, reason: r.error.message };
  if (r.signal || r.status == null) {
    return { unknown: true, reason: `docker --context ${ctx} did not answer within ${timeout}ms` };
  }
...
async function checkSharedDevDb(projectId, machine) {
  const row = await sharedDevDb(projectId, machine.docker_ctx);
  if (!row) {
...
export async function probeBuildReadiness(machine, project, { docker = dockerAdapter } = {}) {
  ...
  checks.push(await checkSharedDevDb(project.id, machine));
